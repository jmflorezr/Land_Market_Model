...
# Imputando el valor 1 a los valores menores a 1 en la columna 'Coca'

# df_panel.loc[df_panel['Coca'] < 1, 'Coca'] = 1
# df_panel.loc[df_panel['Permanentes'] < 1, 'Permanentes'] = 1
# df_panel.loc[df_panel['Transitorios'] < 1, 'Transitorios'] = 1
# df_panel.loc[df_panel['COLOCACIONES'] < 1, 'COLOCACIONES'] = 1

# import numpy as np

# # Suponiendo que df_panel es tu DataFrame
# # Aplicando el logaritmo natural a las columnas especificadas

# columns_to_log = ["Predios_Cambio"]

# # columns_to_log = ["Predios_Cambio", "PRurales", "Coca", "Permanentes", 
# #                   "Transitorios", "PoblCabecera", "PoblRural", "COLOCACIONES"]

# for column in columns_to_log:
#     # Asegurándonos de que no hay valores no positivos en las columnas
#     if (df_panel[column] <= 0).any():
#         raise ValueError(f"La columna {column} contiene valores no positivos, no se puede aplicar logaritmo")

#     df_panel[column] = np.log(df_panel[column])

# Ahora df_panel tiene las columnas transformadas
###No se aplica porque baja el R^2 y no corrige la distribucion de los errores
#aunque si mejora la distribucion
...
# # Asumimos que los residuos están en un array llamado 'residuals'
# # y las variables independientes están en un DataFrame llamado 'independent_vars'

# white_test = het_white(residuals, independent_vars)
...
formula = 'Predios_Cambio ~ 1 + Permanentes + PoblRural + act_secundaria + par_agreg + Ind_Envej_Urb + EntityEffects'
model = PanelOLS.from_formula(formula, data=df_panel)
results = model.fit()
print(results)

# Preparación de datos para pruebas posteriores
# Extraer los residuos y las variables independientes del modelo
columns_to_extract = ["Predios_Cambio", "Permanentes", "PoblRural", "act_secundaria", "par_agreg", "Ind_Envej_Urb"]
df_panel2 = df_panel[columns_to_extract].copy()

residuals = results.resids
independent_vars = df_panel2.drop(['Predios_Cambio'], axis=1)


from statsmodels.stats.outliers_influence import variance_inflation_factor
# Obtener las variables independientes de forma adecuada para el cálculo del VIF
#Prueba para multicolinealidad
vif = pd.DataFrame()
...
print('VIF results:', vif)

# Prueba de Normalidad de los Errores (Shapiro-Wilk)
from scipy import stats
shapiro_test = stats.shapiro(results.resids)
print('Shapiro-Wilk test:', shapiro_test)
# Histograma de residuos
import matplotlib.pyplot as plt
plt.hist(results.resids, bins=30)
plt.title('Histogram of Residuals')
plt.show()

# Prueba de Autocorrelación (Durbin-Watson)
from statsmodels.stats.stattools import durbin_watson
dw_test = durbin_watson(results.resids)
print('Durbin-Watson test:', dw_test)

# Prueba de Estacionariedad (Dickey-Fuller Aumentada)
from statsmodels.tsa.stattools import adfuller
adf_test = adfuller(df_panel['Predios_Cambio'])
print('Dickey-Fuller test:', adf_test)
# Gráfica de la serie temporal
df_panel['Predios_Cambio'].plot()
plt.title('Time Series of Predios_Cambio')
plt.show()

from linearmodels.panel import compare
from statsmodels.stats.diagnostic import het_white, het_breuschpagan

# # Prueba de Homocedasticidad (Test de White o Breusch-Pagan)
# # Nota: 'independent_vars' deben incluir una constante para estos tests
# exog = sm.add_constant(independent_vars)
...
###############################
##Modelo Inicial, Revivar R2 0.08
###############################
...
independent = df_panel[['PoblRural', 'act_secundaria', 'par_agreg', 'Ind_Envej_Urb']]
