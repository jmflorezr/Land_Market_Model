...
def crear_modelo():
    model = keras.Sequential([
        layers.Dense(25,  activation="relu", name="capa-1-oculta-39-neuronas"),
        layers.Dropout(0.3),
        layers.Dense(10, activation="relu", name="capa-2-oculta-15-neuronas"),
        layers.Dropout(0.2),
...
# Añadimos la columna 'COD_MPIO' a la lista
top_features_list.append('COD_MPIO')

# Extraemos las columnas deseadas para crear un nuevo DataFrame
df220 = df210[top_features_list]
df230 = df220.drop(columns=['COD_MPIO', 'clase'])
Experimentos = df230
y_predRNN = Classifier_TF_NN.predict(Experimentos)

y_predRNN01 = pd.DataFrame(y_predRNN)
df10000 = pd.DataFrame(top_features_list)
# Unir los dos DataFrames
resultadoRNN = pd.concat([df220, y_predRNN01], axis=1)

# Exportar a Excel
resultadoRNN.to_excel('resultadoRNN.xlsx', index=False, engine='openpyxl')
